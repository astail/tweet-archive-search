# /// script
# requires-python = ">=3.11"
# dependencies = [
#     "meilisearch",
#     "python-dotenv",
# ]
# ///
"""Create (if needed) and configure the 'tweets' Meilisearch index.

Safe to re-run: index creation is skipped if it already exists, and
settings are applied idempotently.
"""

import os
from pathlib import Path

import meilisearch
from dotenv import load_dotenv

ROOT = Path(__file__).resolve().parent.parent

SETTINGS = {
    "searchableAttributes": ["full_text", "hashtags"],
    "filterableAttributes": ["lang", "is_retweet", "hashtags", "created_at_ts"],
    "sortableAttributes": ["created_at_ts", "favorite_count", "retweet_count"],
    "typoTolerance": {
        "enabled": True,
        # Meilisearch defaults (5/9 chars) are tuned for English. Japanese
        # word tokens are frequently 2-4 characters, so typo tolerance
        # barely engages unless these thresholds are lowered.
        "minWordSizeForTypos": {"oneTypo": 3, "twoTypos": 7},
    },
}


def main():
    load_dotenv(ROOT / ".env")
    host = os.environ.get("MEILI_HOST", "http://localhost:7700")
    master_key = os.environ["MEILI_MASTER_KEY"]
    client = meilisearch.Client(host, master_key)

    try:
        task = client.create_index("tweets", {"primaryKey": "id"})
        client.wait_for_task(task.task_uid)
        print("Created index 'tweets'.")
    except meilisearch.errors.MeilisearchApiError as e:
        if "index_already_exists" not in str(e):
            raise
        print("Index 'tweets' already exists.")

    index = client.index("tweets")
    task = index.update_settings(SETTINGS)
    client.wait_for_task(task.task_uid)
    print("Applied index settings.")


if __name__ == "__main__":
    main()
